...
def non_max_suppression(detections, iou_thresh=0.2): # Thresh menor para ser mais agressivo
    ...
    while detections:
        ...
        keep.append(best)
        detections = [d for d in detections if get_iou((best["x"], best["y"]), (d["x"], d["y"])) < iou_thresh]
    return keep
...
def detect_digits(image, model, device, stride=2, batch_size=128):
    ...
    for y in range(0, h - 28 + 1, stride):
        for x in range(0, w - 28 + 1, stride):
            ...
            # --- MELHORIA 1: Filtro de Intensidade ---
            # Se o crop for quase todo escuro, ignoramos (fundo óbvio)
    ...
    for i in range(0, len(crops_t), batch_size):
        ...
        with torch.no_grad():
            ...
            # Pegamos as duas melhores classes para calcular a margem
            top_probs, top_classes = torch.topk(probs, k=2, dim=1)
            
            confs = top_probs[:, 0]
            margin = top_probs[:, 0] - top_probs[:, 1] # Gap entre 1º e 2º
...
def main():
    ...
    for p in img_paths[:10]:
        ...
        # Usar stride 2 para não perder o centro do dígito
