...
def read_labels(p: pathlib.Path):
    ...
    with open(p, "r", encoding="utf-8") as f:
        for line in f.readlines()[1:]:
            ...
            if len(parts) != 5:
                ...
                ok = False
            if len(parts) != 5:
                raise ValueError(f"Linha inválida em {p}: {line}")
...
def list_image_paths(dataset_dir: pathlib.Path):
    imgdir = dataset_dir / "images"
    paths = list(imgdir.glob("*.png"))
    try:
...
def collect_dataset_stats(dataset_dir: pathlib.Path, n_classes=10):
    imgs = list_image_paths(dataset_dir)
    labdir = dataset_dir / "labels"
    ...
    return {
        ...
        "digits_per_image": {
            "mean": m(dpi.astype(np.float32)),
            "median": md(dpi.astype(np.float32)),
            "min": int(dpi.min()) if dpi.size else 0,
            "max": int(dpi.max()) if dpi.size else 0,
        },
...
def draw_bboxes(ax, bboxes, labels, color=BOX_COLOR):
...
def save_mosaic(dataset_dir: pathlib.Path, out_path: pathlib.Path, n=16, cols=4, seed=0, max_images=None):
    imgs = list_image_paths(dataset_dir)
    if max_images is not None:
        imgs = imgs[:max_images]
    if not imgs:
        raise RuntimeError(f"Sem imagens em {dataset_dir}/images")
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        if lp.is_file():
            try:
                labels, bboxes, _ = read_labels(lp)
                draw_bboxes(ax, bboxes, labels, BOX_COLOR)
                ax.set_title(f"{impath.stem} ({len(labels)})", fontsize=9)
            except Exception:
                h, w = im.shape[0], im.shape[1]
                ax.add_patch(patches.Rectangle((0, 0), w - 1, h - 1, fill=False, linewidth=2.2, edgecolor=BOX_COLOR))
...
def plot_class_distribution(stats_list, out_path: pathlib.Path, n_classes=10):
    ...
    ax.set_xticks(x); ax.set_xlabel("Classe (dígito)"); ax.set_ylabel("Contagem"); ax.set_title("Distribuição de classes")
    if len(stats_list) > 1:
...
def plot_digits_per_image_hist(stats, out_path: pathlib.Path):
    ...
    if hist:
        ...
        ax.bar(ks, vs)
        ax.set_xlabel("Nº de dígitos na imagem"); ax.set_ylabel("Nº de imagens"); ax.set_title("Histograma do nº de dígitos por imagem")
    else:
...
def plot_bbox_size_hist(dataset_dir: pathlib.Path, out_path: pathlib.Path):
    areas = []
    labdir = dataset_dir / "labels"
    for impath in list_image_paths(dataset_dir):
        lp = labdir / f"{impath.stem}.txt"
        if not lp.is_file():
            continue
        _, b, _ = read_labels(lp)
        if b.size:
            areas += bbox_wh_area(b)[2].tolist()

    fig, ax = plt.subplots(figsize=(7.5, 4.2))
    if areas:
        ax.hist(np.asarray(areas, np.float32), bins=30)
        ax.set_xlabel("Área da bbox (px²)"); ax.set_ylabel("Frequência"); ax.set_title("Histograma do tamanho dos dígitos (área das bboxes)")
    else:
...
def plot_mean_bbox_area_per_class(stats, out_path: pathlib.Path, n_classes=10):
    ...
    ax.bar(x, mean_area)
    ax.set_xticks(x); ax.set_xlabel("Classe (dígito)"); ax.set_ylabel("Área média (px²)"); ax.set_title("Tamanho médio por classe (área da bbox)")
    fig.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
...
def _pretty_title(p: pathlib.Path):
    m = {
...
def save_statistics_mosaic(pngs, out_path: pathlib.Path, suptitle="Estatísticas", ncols=2):
    pngs = [p for p in pngs if p.is_file()]
    if not pngs:
        return
    n = len(pngs); ncols = max(1, int(ncols)); nrows = int(math.ceil(n / ncols))
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        ax.imshow(img); ax.axis("off"); ax.set_title(_pretty_title(pngs[i]), fontsize=12)
...
def main():
    ...
    ap.add_argument("-od", "--outdir", default="./dataset_stats_out")
    ...
    for d in args.directories:
        ...
        if dd.resolve() == primary.resolve():
            primary_stats = st
        print(f"\nDataset: {dd}")
        print(f" - Nº total de imagens analisadas: {st['num_images']}  |  Labels em falta: {st['missing_label_files']}")
        print(f" - Nº total de digitos criados: {st['num_digits_total']}")
        print(f"  Outputs: {outdir}\n")

    with open(outdir / "stats_summary.json", "w", encoding="utf-8") as f:
        json.dump({"primary_dataset": str(primary), "datasets": stats_list}, f, indent=2, ensure_ascii=False)

    save_mosaic(primary, outdir / "mosaic_with_bboxes.png", n=args.mosaic_num, cols=args.mosaic_cols, seed=args.seed, max_images=args.max_images)

    class_png = outdir / "class_distribution.png"
    plot_class_distribution(stats_list, class_png)

    if primary_stats is None:
        primary_stats = stats_list[0]

    tmp = outdir / "estatisticas_graficos"; tmp.mkdir(parents=True, exist_ok=True)
    p1, p2, p3 = tmp / "digits_per_image_hist.png", tmp / "bbox_area_hist.png", tmp / "mean_bbox_area_per_class.png"
    plot_digits_per_image_hist(primary_stats, p1)
    plot_bbox_size_hist(primary, p2)
    plot_mean_bbox_area_per_class(primary_stats, p3)

    save_statistics_mosaic([p1, p2, p3, class_png], outdir / "estatisticas.png", suptitle="Estatísticas", ncols=2)
